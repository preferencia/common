...
int hide_banner = 0;
AVRational  g_TimeBaseQ = {1, AV_TIME_BASE};
                        ...
                        const char *arg)
{
    ...
    if (po->flags & OPT_STRING) {
        ...
        *(double *)dst = parse_number_or_die(opt, arg, OPT_DOUBLE, -INFINITY, INFINITY);
        ...
        int ret = po->func_arg(optctx, opt, arg);
        if (ret < 0) {
			char szErrMsg[AV_ERROR_MAX_STRING_SIZE] = {0, };
            av_log(NULL, AV_LOG_ERROR,
                   "Failed to set value '%s' for option '%s': %s\n",
                   arg, opt, av_make_error_string(szErrMsg, AV_ERROR_MAX_STRING_SIZE, ret));
            return ret;
        }
    }
...
static void expand_filename_template(AVBPrint *bp, const char *pTemplate,
                                     struct tm *tm)
{
    int c;
...
static int init_report(const char *env)
{
        ...
        if ((ret = av_opt_get_key_value(&env, "=", ":", 0, &key, &val)) < 0) {
            if (count)
			{
                ...
                av_log(NULL, AV_LOG_ERROR,
                       ...
                       av_make_error_string(szErrMsg, AV_ERROR_MAX_STRING_SIZE, ret));
			}
            break;
        }
        if (*env)
    ...
    expand_filename_template(&filename,
                             (const char *)av_x_if_null(filename_template, (const char *)("%p-%t.log")), tm);
    av_free(filename_template);
